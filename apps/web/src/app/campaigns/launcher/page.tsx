...
import { TCPAConsentAcknowledgment, LegalDisclaimerBanner, RealEstateDisclaimer } from '@/components/compliance';
...
export default function CampaignLauncherPage() {
  ...
  const [showComplianceWall, setShowComplianceWall] = useState(false);
  const [complianceAcknowledged, setComplianceAcknowledged] = useState(false);

  // Check if user has already accepted messaging compliance
  const { data: hasMessagingAcceptance } = useQuery({
        ...
        <Card className="border-[var(--color-success)]/30 bg-[var(--color-success)]/10">
          <CardContent className="pt-6">
            <div className="text-center">
              <CheckCircle2 className="h-16 w-16 text-[var(--color-success)] mx-auto mb-4" />
              <h2 className="text-2xl font-bold text-[var(--color-success)] mb-2">Campaign Launched!</h2>
              <p className="text-[var(--color-success)]/80 mb-6">
                Your {targetDeals}-deal campaign is now active. The system will automatically:
              </p>
              <ul className="text-left text-[var(--color-success)] space-y-2 mb-6 max-w-md mx-auto">
              ...
              <div className="flex gap-4 justify-center">
                <Link href="/campaigns/monitor">
                  <Button className="btn-gradient">Monitor Campaign</Button>
                </Link>
                <Link href="/analytics">
                  <Button variant="outline" className="border-[var(--border-subtle)] text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]">View Analytics</Button>
                </Link>
              </div>
            </div>
          </CardContent>
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="mb-6">
        <Link href="/campaigns" className="text-sm text-[var(--text-muted)] hover:text-[var(--text-secondary)] flex items-center gap-1">
          ...
          <div className="space-y-8">
            <div>
              <label className="block text-sm font-medium text-[var(--text-secondary)] mb-4">
                Target Assignment Contracts: <span className="text-2xl font-bold text-[var(--accent-blue)]">{targetDeals}</span>
              </label>
              <Slider
                value={[targetDeals]}
            ...
            {plan && (
              <>
                ...
                <div className="bg-[var(--bg-tertiary)] rounded-lg p-4 border border-[var(--border-subtle)]">
                  <h3 className="font-medium text-[var(--text-primary)] mb-3">Campaign Timeline</h3>
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-2xl font-bold text-[var(--text-primary)]">{plan.dailyOutreach.toLocaleString()}</div>
                      <div className="text-xs text-[var(--text-muted)]">Daily outreach capacity</div>
                    </div>
                    <div>
                      <div className="text-2xl font-bold text-[var(--text-primary)]">{plan.daysToTarget}</div>
                      <div className="text-xs text-[var(--text-muted)]">Days to complete</div>
                    </div>
                    <div>
                      <div className="text-2xl font-bold text-[var(--text-primary)]">{((plan.sellersNeeded + plan.buyersNeeded) * TOUCHES_PER_LEAD).toLocaleString()}</div>
                      <div className="text-xs text-[var(--text-muted)]">Total touches</div>
                    </div>
                  </div>
                </div>

                {inventory && (
                  <div className="bg-[var(--accent-blue)]/10 rounded-lg p-4 border border-[var(--accent-blue)]/30">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-[var(--text-secondary)]">Seller Leads</span>
                        <div className="flex items-center gap-2">
                          {inventory.sellers.available >= inventory.sellers.needed ? (
                            <Badge variant="outline" className="bg-[var(--color-success)]/10 text-[var(--color-success)] border-[var(--color-success)]/30">
                              {inventory.sellers.available.toLocaleString()} available
                            </Badge>
                          ) : (
                            <Badge variant="outline" className="bg-[var(--color-warning)]/10 text-[var(--color-warning)] border-[var(--color-warning)]/30">
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-[var(--text-secondary)]">Buyer Leads</span>
                        <div className="flex items-center gap-2">
                ...
                <div className="bg-[var(--color-success)]/10 rounded-lg p-4 border border-[var(--color-success)]/30">
                  <h3 className="font-medium text-[var(--text-primary)] mb-3">What Happens When You Launch</h3>
                  <div className="grid md:grid-cols-2 gap-4 text-sm">
                    <div>
                      <h4 className="font-medium text-[var(--color-success)] mb-2">Seller Pipeline</h4>
                      <ul className="space-y-1 text-[var(--color-success)]/80">
                    ...
                    <div>
                      <h4 className="font-medium text-[var(--color-success)] mb-2">Buyer Pipeline</h4>
                      <ul className="space-y-1 text-[var(--color-success)]/80">
            ...
            {errorMsg && (
              <div className="bg-[var(--color-error)]/10 text-[var(--color-error)] p-4 rounded-lg flex items-center gap-2 border border-[var(--color-error)]/30">
                <AlertTriangle className="h-5 w-5" />
                {errorMsg}
              </div>
            )}

            {/* Compliance Acknowledgment - show before launch if not yet accepted */}
            {showComplianceWall && !hasMessagingAcceptance && (
              <TCPAConsentAcknowledgment
                contactCount={(plan?.sellersNeeded || 0) + (plan?.buyersNeeded || 0)}
                consentMode="unverified"
                onAccept={() => {
                  setComplianceAcknowledged(true);
                  setShowComplianceWall(false);
                  launchMutation.mutate();
                }}
                onDecline={() => setShowComplianceWall(false)}
              />
            )}
            ...
            {!showComplianceWall && (
              <Button
                ...
                onClick={() => {
                  if (hasMessagingAcceptance || complianceAcknowledged) {
                    launchMutation.mutate();
                  } else {
                    setShowComplianceWall(true);
                  }
                }}
      ...
      {/* Platform Disclaimer */}
      <LegalDisclaimerBanner variant="dismissible" showOnce className="mt-6" />
    </div>
  );
}
